...
const Live = ({ darkMode }) => {
    ...
    return (
        <div className={`min-h-screen flex flex-col items-center justify-start ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {liveEvents.length > 0 ? (
                            liveEvents.map((event) => (
                                <div key={event._id} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                                    <div className="flex flex-col items-center">
                                        {event.onlinePoster && (
                                            <img src={event.onlinePoster} alt="Online Poster" className="w-full h-48 object-cover" />
                                        )}
                                        {event.offlinePoster && (
                                            <img src={event.offlinePoster} alt="Offline Poster" className="w-full h-48 object-cover" />
                                        )}
                                    </div>
                                    <div className="p-6">
                                        ...
                                        {event.link && (
                                            <a href={event.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mt-4 block">
                                            Register Here
                                            </a>
                                        )}
